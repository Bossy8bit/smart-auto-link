...
function resolveLanguage(language) {
  ...
  return (0, import_obsidian.getLanguage)().toLowerCase().startsWith("th") ? "th" : "en";
}
...
function autoLink(content3, entries, options) {
  ...
  const protect = (pattern) => {
    for (const m of content3.matchAll(pattern)) protectedRanges.push([m.index, m.index + m[0].length]);
  };
  protect(/^\uFEFF?---[^\S\r\n]*\r?\n[\s\S]*?(?:\r?\n(?:---|\.\.\.)[^\S\r\n]*(?=\r?\n|$)|$)/g);
  protect(/!?\[\[[\s\S]*?(?:\]\]|$)/g);
  protect(/%%[\s\S]*?(?:%%|$)/g);
  ...
  function visit(node2) {
    ...
    } else for (const child of (_c = node2.children) != null ? _c : []) visit(child);
  }
  visit(tree);
  let output = content3;
...
function contentTerms(content3, minimumLength, minCount = 3, maxTerms = 5) {
...
var SmartAutoLinkPlugin = class extends import_obsidian2.Plugin {
  ...
  autoLink(content3, entries) {
    return autoLink(content3, entries, this.settings);
  }
};
var AutoLinkSettingTab = class extends import_obsidian2.PluginSettingTab {
